  ...
  return (
    <>
      ...
      <button
        ...
        className="lg:hidden fixed top-4 left-4 z-50 p-2.5 rounded-xl bg-[var(--bg-card)] border border-[var(--border-card)]"
        ...
        <div className="p-5 border-b border-[var(--border-card)]">
          <Link href="/dashboard" className="flex items-center gap-2.5" onClick={() => setIsOpen(false)}>
            ...
            <div>
              ...
              <span className="font-display font-semibold text-emerald-500"> AI</span>
            </div>
          </Link>
        </div>
      ...
      {/* Mobile overlay */}
      {isOpen && (
        <div className="fixed inset-0 bg-black/40 z-30 lg:hidden animate-fade-in" onClick={() => setIsOpen(false)} />
      )}
    </>
  );
}